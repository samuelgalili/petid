...
## Exposure measurement — implemented

| ID | Decision |
|---|---|
| **PD-45** | **Legacy Exposure Must Be Measured Before Enforcement.** No public hiding, checkout blocking, migration, reassignment or archival may be decided before the exposure report has been run against production. Enforcement without measurement risks emptying the shop or stopping revenue by an unknown amount; of the six surfaces that must be measured first, only order history is currently cleared.<br><br>**Measurable** (awaiting a production run): population, public exposure, purchase exposure including an upper bound on variant-price exposure, order exposure, ownership-review counts, content provenance, and breakdowns by business, category, month and order exposure.<br><br>**`UNRECONSTRUCTIBLE`, returned with no number attached:** which products came through the legacy intake path, which carry a fallback `business_id`, who owns anything, and publication state — the first three were never recorded, the fourth has no column. **`UNMEASURABLE_SERVER_SIDE`:** cart exposure, because carts live in each browser's `localStorage` with no cart table and no telemetry — a structural impossibility, not a missing query, and telemetry was out of scope here.<br><br>**`source_url` and `supplier_id` are not ownership evidence.** The first states where the content came from; the second is a procurement reference with no foreign key, set only by a dead screen. Both are read by the tool, neither as ownership. **Order history stays independent:** `order_items` has one foreign key, to `orders.id`, and the dangling-reference count is a planning metric — a dangling `product_id` is the correct state of an immutable snapshot, and no runtime code performs that join.<br><br>**The route is read-only and writes nothing, including no audit row for the read** — making a measurement `GET` the only writing `GET` in the API would be a worse precedent than the traceability it buys. **No migration:** every measurable dimension uses columns that already exist, and every dimension needing a new column is one this checkpoint may not invent.<br><br>**No figure produced against a non-production database may be recorded or cited as exposure.** `production_validated` is hardcoded `false` and nothing in the codebase can set it true.<br><br>**Blocked until production access exists:** every count above, and therefore the sequencing decision between blocking checkout, hiding the catalogue, migrating verified products, or treating only commercially-exposed products. |
