...
קצה פרודקשן: `3e3a25867c30b7050c1cf8a461846f1d06014864`
ריצה אחרונה: run 50 · `success` · `Deploy tested build`: success
נבדק ב-2026-09-14
...
**כל השאלות הפתוחות מרוכזות ב-`docs/OPEN-QUESTIONS.md`** — 20 שאלות, שלוש
מהן חוסמות, לכל אחת המלצה ומה היא חוסמת. הטבלה כאן היא תת-קבוצה: הממצאים
שמופו ולא תוקנו.

חמישה ממצאים ממופים ולא מתוקנים. לכל אחד ההחלטה שהוא ממתין לה.

| ממצא | מה | מה נדרש כדי לסגור |
|---|---|---|
| `F-007` | 20 עמודות מתות ב-`profiles` ו-`app_users` | **החלטה + בדיקת נתונים** — ראה האזהרה למטה |
| `F-008` | `show_location` נקראת בשאילתה הציבורית ואף אחד לא כותב אליה, אז עיר הבעלים לעולם לא מוצגת | **החלטה מוצרית** — לבנות פקד "הצג את עירי" במודעת אבידה, או להסיר את הענף המת |
| `F-009` | להסכמה אין provenance. `consent_method` ריקה, וההיסטוריה מוגבלת למעבר האחרון | **החלטה** — למלא את `consent_method` ולרשום שינויים כאירועים, או להעביר הסכמה לעובדות |
| `F-012` | 4 עמודות מתות ב-`business_products` | **החלטה** — האם נבנית מערכת ביקורות |
| `F-013` | 10 עמודות מתות ב-`business_profiles`, כולל זרימת אישור בת 3 עמודות בלי מאשר | **החלטה** — האם Mipo תציג ספקים חיצוניים |

### אזהרה לפני שמוחקים עמודה כלשהי

**מחיקת עמודה היא בלתי הפיכה, והנתונים שבה נעלמים.** שלוש מ-20 העמודות
המתות — `legacy_auth_provider`, `legacy_user_id`, `imported_at` — הן עמודות
ההגירה מ-Supabase, ו-`points`, `interests` ו-`favorite_breeds` עשויות להחזיק
נתוני משתמשים אמיתיים מאותה תקופה.

**לא ניתן לאמת מכאן מה יש בהן בפרודקשן.** לפני כל `drop column` צריך לספור
שורות לא-ריקות בכל אחת מהן על בסיס הנתונים החי. זו הסיבה שלא נכתבה מיגרציית
מחיקה בשלב הזה — לא חוסר זמן, אלא שמחיקה עיוורת היא הדבר היחיד כאן שלא ניתן
לבטל.

המסלול הבטוח: לספור → להחליט לכל עמודה → לגבות → למחוק במיגרציה אחת עם
rollback מתועד בכותרת.
...
### 2026-09-14 · מודל ממשל הנתונים, ותיקוני F-010 ו-F-011

- **סטטוס:** נפרס ואומת
- **קומיט:** `3e3a25867c30b7050c1cf8a461846f1d06014864` (מיזוג של `f7b208c3`)
- **ענף:** `aws-migration`
- **ריצת deploy:** run 50 · `success` · `Deploy tested build`: success, כולל `Smoke test the deployed environment`
- **שערים:** typecheck ✅ · lint ✅ · imports ✅ · build ✅ · unit **257/257** · migrations **38/38** + הרצה חוזרת no-op · API smoke **19/19**

**מה נעשה.** מופו כל חמש הישויות ב-`58-DATA-GOVERNANCE-MODEL.md`: **224 עמודות
נבדקו, 45 מתות**. ההכרעה — ישות אחת מתוך חמש זקוקה לשכבת עובדות, והיא זו שכבר
יש לה אחת. נפתחו `F-007` עד `F-013`.

תוקן `F-011`: `feeding_guide_source` התקבל, אומת לשלושה מצבים, ולא נכתב ביצירה.
נוספה עמודה אחת ל-`productColumns` ובדיקת רגרסיה ל-`db-smoke.mjs`. **הוכח
שהבדיקה תופסת**: עם הקוד הקודם 18/19, עם התיקון 19/19.

`db-smoke.mjs` קיבל גם מפתח אדמין שנוצר לכל ריצה, כדי שבדיקות מסלולי האדמין
ירוצו במקום לדלג בשקט.

**אבד ושוחזר.** הקונטיינר אותחל בין הסשנים ו-`03-PET-LIFECYCLE.md` נמחק — הוא
נכתב אך לא קומט, כי שאלתי "לקמט?" במקום לקמט. שוחזר במלואו ב-`8bd1d3b6`.
**לקח שנכנס לפרוטוקול: תיעוד נדחף לענף מיד. הענף אינו פרודקשן, והאישור נדרש רק
שם.**

**`F-010` תוקן.** הוחלט שאין צורך באימות ת״ז. `id_verified` הוסר משלושת
המקומות ב-`OwnerProfile`, מ-`serializeProfile` ומטיפוס `MipoProfile` — אפס
אזכורים נותרו. **`id_number_last4` נשאר במכוון**: תביעת ביטוח נושאת אותו
ב-`insurance_claims.owner_id_number`, ומחיקה גורפת הייתה שוברת הגשת תביעה.

- **לא בוצע:** `F-007`, `F-008`, `F-009`, `F-012`, `F-013` — מופו ולא תוקנו
- **לא אומת:** אין. כל טענה במסמך נגזרה מ-`information_schema` או מספירת
  אזכורים מעל `server/src` עם הערות מנוכות
- **הצעד הבא:** DUAL WRITE — לחבר כותב אחד לשכבת העובדות, מועמד ראשון: משקל

**תיקוני עצמי ברשומה הזו** — שלושה, כולם נתפסו בהרצה ולא בקריאה:
1. אמרתי ש-`profiles` יש 38 עמודות. ספרתי ברשימה מופרדת ברווחים בעין.
   `information_schema` אומר **39**.
2. סימנתי חמש עמודות כ"נקראות ולא נכתבות". שלוש היו שגויות — ה-regex דרש
   `= $N` ופספס `= now()` ו-`= case when`.
3. כמעט דיווחתי על נתוני דירוג מומצאים ב-`AIAnalytics.tsx` כבאג גלוי. לרכיב
   **אפס importers**. חוב, לא באג.
