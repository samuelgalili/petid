/**
 * Matching what a person typed against what the catalogue holds.
 *
 * The shop is one question, so this is the thing that answers it.
 *
 * ─── WHAT THE FIRST TWO VERSIONS GOT WRONG ──────────────────────────────────
 *
 * V1 searched the product NAME only and matched the WHOLE QUERY as one
 * substring. The owner broke it in a minute: "מזון יבש לכלב" -> nothing,
 * "מזון יבש כלב" -> results.
 *
 * V2 (the fix for that) searched a wide haystack with every token required.
 * Better, and still wrong in ways a measurement found immediately. Run against
 * the real catalogue rows, ALL THREE of the shop's own suggestion chips - the
 * buttons on the resting screen, the first thing a visitor taps - returned
 * nothing:
 *
 *   "הכלב שלי משיר הרבה"  -> 0      "שלי" and "הרבה" were required words
 *   "אוכל יבש לגור"        -> 0      the catalogue says מזון, the shopper says אוכל
 *   "צעצוע לתוכי"          -> 0      pet_type has no parrot, and 'all' mapped to nothing
 *
 * and "כדור" returned a CAT FOOD, because the query was prefix-stripped to the
 * fragment "דור" and that fragment appeared in the middle of "בכדורי שיער".
 *
 * Those four results are the whole specification for this file.
 *
 * ─── WHAT IT DOES INSTEAD ───────────────────────────────────────────────────
 *
 * 1. WORDS, NOT SUBSTRINGS. V2 joined every field into one string and asked
 *    `includes`. That is why a fragment matched mid-word. Here both sides are
 *    split into words and words are compared to words, so a match is always a
 *    whole word on both ends.
 *
 * 2. BOTH SIDES ARE STEMMED. V2 stripped prefixes off the QUERY only, so it
 *    could find "כלב" inside "לכלבים" but never the reverse, and "חטיף" found
 *    nothing while "חטיפים" found the treats. Hebrew glues prepositions on the
 *    front and plurals on the back; stemming one side is half an answer.
 *
 * 3. WORDS A SHOPPER USES, NOT WORDS THE CATALOGUE USES. The categories in the
 *    database are literally named "אוכל יבש" and "מזון" - the catalogue itself
 *    cannot agree with itself - so אוכל and מזון have to be the same word here.
 *    That is what CONCEPTS is: not a thesaurus for its own sake, but the list
 *    of places the shopper's vocabulary and the catalogue's diverge.
 *
 * 4. A QUERY IS A SENTENCE, NOT A KEY. "הכלב שלי משיר הרבה" is how a person
 *    asks. STOP_WORDS drops the words that carry no product in them, and a
 *    token that matches nothing in the catalogue is REPORTED AND DROPPED
 *    rather than allowed to zero the whole answer. The screen can then say
 *    which words it actually used, which is the difference between a search
 *    that failed and a search that answered.
 *
 * 5. RANKED, NOT FILTERED. The shop shows twelve cards. A filter returns them
 *    in catalogue order, so the product whose NAME is what you typed can sit
 *    behind eleven products that merely mention it and never appear. A name is
 *    worth more than a description and this says so in numbers.
 *
 * WHY AND IS STILL THE RULE. Every surviving token must match. With OR, each
 * extra word makes the answer noisier; with AND, each extra word narrows it.
 * The relaxation in (4) is what keeps AND from being brittle: words are
 * dropped deliberately and visibly, never silently turned into OR.
 */

// ─── normalising ────────────────────────────────────────────────────────────

/**
 * Hebrew's five final forms, folded to their ordinary letters.
 *
 * This is what lets a stem be compared at all: "חטיף" ends in ף and its plural
 * "חטיפים" has פ in the same position. Without folding, the singular and the
 * plural of the same word have no common prefix, and "חטיף" returns nothing
 * while "חטיפים" returns the treats - which is exactly what it did.
 */
...
// ─── exported surface ───────────────────────────────────────────────────────
//
// EVERYTHING ABOVE THIS LINE IS SHARED VERBATIM with src/lib/catalogSearch.ts,
// the copy the browser runs, and catalogSearch.test.js compares the two
// character for character. Two searches that disagree is a shop that finds a
// product on one screen and denies it on another.
//
// Only this tail differs, and only because the browser's copy carries the
// TypeScript types. Nothing that decides a result belongs below here.

/** Every word of a product, flattened. */
export const productHaystack = (product) => haystackOf(product);

/** The words of a query, each carrying what it could mean. */
export const queryTokens = (query) => tokensOf(query);

/** The catalogue, answered - with what was matched and what was not. */
export const searchCatalogDetailed = (products = [], query) => searchDetailed(products, query);

/** Does this product answer that query. */
export const productMatchesQuery = (product, query) => searchDetailed([product], query).results.length > 0;

/** The catalogue, filtered and ranked. */
export const searchCatalog = (products = [], query) => searchDetailed(products, query).results;
