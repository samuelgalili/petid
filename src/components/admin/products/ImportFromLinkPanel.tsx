 ...
 *
 * Four steps: scrape the page and check for a product already like it, read
 * the ingredients and score them, work out kcal and feeding amounts, then
 * attach a supplier and save.
 *
 * IT WAS /admin/quick-import, ONE OF FOUR PRODUCT SCREENS. It is a section of
 * the products screen now, so the list it adds to is one press away rather
 * than a navigation.
 *
 * Note for whoever consolidates further: this scrapes with
 * `smart-scrape-product` while ProductImportWizard's URL path uses
 * `import-products-from-url`. Two backend functions for the same act. Which
 * one survives is not a layout decision and was deliberately left alone here.
 */
export const ImportFromLinkPanel = ({ onImported, onClose }: {
  /** A product exists now; the list behind this should reload. */
  onImported: () => void;
  onClose: () => void;
}) => {
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      updateStatus(4, "done");
      // The list behind this panel is now stale by exactly one product.
      onImported();
            ...
            {stepStatus[4] !== "done" ? (
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  {/* Back to the list, which this panel is a section of now. */}
                  <Button variant="ghost" onClick={onClose} className="gap-2 text-base font-bold" size="lg">
                    לרשימת המוצרים
                  </Button>
                </div>
              </motion.div>
...
export default ImportFromLinkPanel;
