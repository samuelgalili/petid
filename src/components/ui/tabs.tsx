...
const TabsList = React.forwardRef<React.ElementRef<typeof TabsPrimitive.List>, React.ComponentPropsWithoutRef<typeof TabsPrimitive.List>>(({
  className,
  ...props
}, ref) => <TabsPrimitive.List ref={ref} className={cn("flex overflow-x-auto scrollbar-hide px-4 py-3 gap-2.5 bg-gradient-to-b from-background to-muted/30 border-b border-border/30", className)} {...props} />);
...
const TabsTrigger = React.forwardRef<React.ElementRef<typeof TabsPrimitive.Trigger>, TabsTriggerProps>(({
  ...
  "group relative flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all duration-300 min-w-fit",
  "bg-muted/40 text-muted-foreground border border-transparent",
  "hover:bg-muted hover:text-foreground hover:border-border/50 hover:shadow-sm hover:-translate-y-0.5",
  "active:scale-95 active:shadow-none",
  "data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:border-primary/20 data-[state=active]:shadow-lg data-[state=active]:shadow-primary/25",
...
const AnimatedTabsTrigger = React.forwardRef<React.ElementRef<typeof TabsPrimitive.Trigger>, AnimatedTabsTriggerProps>(({
  ...
  return <TabsPrimitive.Trigger ref={node => {
    ...
    "relative flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all duration-300 min-w-fit",
    "bg-muted/40 text-muted-foreground border border-transparent",
    "hover:bg-muted hover:text-foreground hover:border-border/50 hover:shadow-sm hover:-translate-y-0.5",
    ...
    className
      ...
      {isActive && <motion.div 
        layoutId={layoutId} 
        className="absolute inset-0 bg-gradient-to-r from-primary via-primary to-primary/90 rounded-xl shadow-lg shadow-primary/30" 
        transition={{
          type: "spring",
          bounce: 0.2,
          duration: 0.5
