...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.List
    ref={ref}
    className={cn(
      "flex overflow-x-auto scrollbar-hide px-3 py-2 gap-2 bg-background/95 backdrop-blur-xl border-b border-border/20",
      className,
    )}
    {...props}
  />
...
>(({ className, icon, children, ...props }, ref) => (
  <TabsPrimitive.Trigger
    ref={ref}
    className={cn(
      "group relative flex items-center gap-1.5 px-4 py-2 rounded-2xl text-sm font-medium whitespace-nowrap transition-all min-w-fit",
  ...
  >
    ...
    <span className="absolute inset-0 rounded-2xl opacity-0 group-data-[state=active]:opacity-100 bg-gradient-to-r from-primary to-primary-light shadow-md transition-opacity duration-200" />
...
>(({ className, icon, children, layoutId = "activeTab", ...props }, ref) => {
  ...
  return (
    ...
    >
      {isActive && (
        <motion.div
          layoutId={layoutId}
          className="absolute inset-0 bg-gradient-to-r from-primary to-primary-light rounded-2xl shadow-md"
