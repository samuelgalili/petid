  ...
  const fetchProducts = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('scraped_products').select('id, product_name, main_image_url, final_price, pet_type').eq('pet_type', pet.type).neq('stock_status', 'out_of_stock').limit(4);
  ...
  if (loading) {
    return <div className="px-4 py-6">
        ...
        <div className="flex gap-3 overflow-x-auto pb-2">
          {[1, 2, 3, 4].map(i => <div key={i} className="w-[100px] flex-shrink-0">
