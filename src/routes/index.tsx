...
const Admin = ({ children, permission }: { children: React.ReactNode; permission?: AdminPermission | AdminPermission[] }) => (
...
export const businessRoutes: RouteObject[] = [
  ...
  { path: "/product-sourcing", element: <Navigate to="/admin/products?section=import" replace /> },
...
const legacyAdminRedirects = legacyAdminPaths.map((path) => {
  ...
  if (path === "/admin/data-import") {
    return { path, element: <Navigate to="/admin/products?section=import" replace /> };
  }
...
export const adminRoutes: RouteObject[] = [
  ...
  // from that role when its screen was folded into this one.
  { path: "/admin/products", element: <AdminPage component={AdminProducts} pageName="מוצרים" permission={[ADMIN_PERMISSIONS.PRODUCTS_READ, ADMIN_PERMISSIONS.INTAKE_READ]} /> },
  // Publishing and the "smart editor" are sections of the products screen
  // now. Redirected rather than removed: these links are in people's
  // history and in this repository's own audit entries.
  { path: "/admin/publishing", element: <Navigate to="/admin/products?section=publishing" replace /> },
  ...
  { path: "/admin/quick-import", element: <Navigate to="/admin/products?section=import" replace /> },
